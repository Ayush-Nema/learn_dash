...
App with data table
======================
The app contains customised data table.

Source: https://www.youtube.com/watch?v=USTqY4gH_VM
Github repo: https://github.com/Coding-with-Adam/Dash-by-Plotly/tree/master/DataTable
Reference link: https://dash.plotly.com/datatable/reference
"""
...
# Creating id column for convenient interaction
df['id'] = df['iso_alpha3']
...
app.layout = html.Div([
    dash_table.DataTable(
        ...
        data=df.to_dict('records'),  # the content of the table
        editable=True,  # allow editing of data inside the cell
        filter_action='native',  # allow filtering of data by user ('native') or not ('none')
        sort_action='native',  # enables data to be sorted per-column by user ('native') or not ('none')
        sort_mode='single',  # sort across 'multi' or 'single' columns
        column_selectable='multi',  # allow users to select 'multi' or 'single' column(s)
        row_selectable='multi',  # allow users to select 'multi' or 'single' row(s)
        row_deletable=True,  # choose if user can delete the row (True) or not (False)
